...
- **Update — connected 2026-08-15:** Project owner supplied a real Atlas cluster (`cluster0.rllp3bl.mongodb.net`, database user `Joshordin`). Connection string written to `server/.env` (gitignored, confirmed via `git check-ignore`, never appears in any committed file or elsewhere in this document) as `MONGODB_URI=mongodb+srv://Joshordin:<password>@cluster0.rllp3bl.mongodb.net/smartbin?retryWrites=true&w=majority&appName=Cluster0` — the `/smartbin` database name and `retryWrites=true&w=majority` were added explicitly (the string as given had neither). Verified genuinely persistent, not just "connects": restarted the server against the same cluster with no code/data changes and confirmed the seed step did **not** re-run (`[seed] created…` lines were absent on the second boot) and all 10 previously-seeded bins were still present via a live API call — this is real, persistent storage, not the throwaway `mongodb-memory-server` instance every prior session used. **Test suite is unaffected and intentionally still uses `mongodb-memory-server`** (hardcoded per-file in `tests/server/*.test.ts`) — tests must never write to the real Atlas database.
...
## 36.9 Deployed: live on Vercel (client) + Render (server) + Atlas (database)

**Decided:** 2026-08-15

Project owner: "let's go ahead and deploy, you have my vercel and render accounts." Confirmed both CLIs (`vercel`, `render`) were already authenticated on this machine (accounts `joshodin2019` / `JOSHODIN2019`), confirmed `gh` (GitHub CLI) authenticated as `JOSHODIN2019`, asked the one genuinely open question — repo visibility — and the project owner chose **public**.

**Live URLs:**
- Client: https://nisepa-smartbin.vercel.app (Vercel project `josh-academy/nisepa-smartbin`)
- Server: https://nisepa-smartbin-server.onrender.com (Render service `nisepa-smartbin-server`, free plan, Oregon region)
- Source: https://github.com/JOSHODIN2019/nisepa-smartbin (public)
- Database: the same real MongoDB Atlas cluster connected in Section 36.1's update — **local dev and the deployed site currently share one database**, not separate dev/prod instances. Worth knowing: bin/waste-level changes made while testing locally are visible on the live site and vice versa.

**Code changes required for a real split deployment** (none of this was needed while everything ran on `localhost`, so none of it existed before today):
- **Auth cookie cross-origin fix.** The cookie was `SameSite=Lax` unconditionally. Vercel and Render are different domains — a genuinely cross-site fetch from the browser's perspective — and Lax cookies are never attached to cross-site fetch/XHR requests, only top-level navigations. Login would have appeared to succeed (the response arrives fine) while every subsequent request silently looked unauthenticated. Fixed: `SameSite=None` (+`Secure`, already conditioned on `NODE_ENV`) in production, `Lax` in dev (`server/src/controllers/auth.controller.ts`). `logout`'s `clearCookie` updated to pass the same attribute object — mismatched attributes between set and clear can leave the original cookie stuck in some browsers.
- **API base URL.** `client/src/lib/api.ts`'s fetch wrapper and `useEventStream`'s raw `EventSource` both hardcoded a relative `/api` path, which only ever resolves correctly same-origin (locally, via Vite's dev proxy). Added `VITE_API_BASE_URL`, a build-time env var (falls back to `/api` when unset, so local dev is untouched), set on Vercel to `https://nisepa-smartbin-server.onrender.com/api`.
- **SSE credentials.** `EventSource` doesn't send cookies cross-origin unless constructed with `{ withCredentials: true }` — unlike `fetch`, which defaults differently. Without this, staff/admin-scoped realtime events would silently never arrive in production (the server can't tell the connection apart from a public one).
- **SPA routing on Vercel.** A direct load (or refresh) of any route other than `/` — e.g. `/login`, `/admin/dashboard` — 404'd, because Vercel was looking for a literal file/route instead of serving `index.html` and letting React Router handle it client-side. Found this by actually testing the deployed site (not just trusting a successful build) — added `client/vercel.json` with a catch-all rewrite to `/index.html`.
- **Render build command.** First deploy failed: `tsc` couldn't find `@types/node`, `@types/express`, etc. Cause: `NODE_ENV=production` was set as a service env var, and npm skips `devDependencies` (where `typescript` and all `@types/*` live) whenever `NODE_ENV=production` during `npm install`. Fixed by changing the build command to `npm install --include=dev && npm run build`, forcing dev deps for the build step regardless of `NODE_ENV` (the compiled output in `dist/` doesn't need them at runtime).

**Known, accepted characteristic — not a bug:** Render's free plan spins a service down after ~15 minutes of inactivity; the first request after that takes several seconds to tens of seconds to respond while it wakes back up (observed `/auth/me` taking ~4.5s on a cold request during verification, versus milliseconds once warm). This produced a real-looking but false alarm during verification — the admin dashboard briefly showed "0 registered bins" and "Loading alerts…" because the screenshot was taken before the cold-started fetches resolved, not because the data or the code was wrong; confirmed by re-testing on a warm instance and by tracing actual fetch timings. No fix applied — this is the standard trade-off for $0/month hosting on Render, consistent with this project's free-first policy (Section 4.3), not something worth adding infrastructure (e.g. a keep-alive ping) to work around for an academic prototype.

**Verification performed against the live deployment** (not just "the build succeeded"): fresh login as all three roles (admin, staff, and the public flow via the unauthenticated Smart Bin page) through the real Vercel URL hitting the real Render URL hitting the real Atlas database; confirmed the auth cookie is actually `SameSite=None` and survives a full page reload; confirmed CORS headers on a live response (`access-control-allow-origin: https://nisepa-smartbin.vercel.app`, `access-control-allow-credentials: true`); confirmed bin data (10 bins, matching the seed) is reachable and renders correctly on both the admin dashboard and the public Smart Bin page; zero console/network errors across all of the above once cold-start timing was accounted for.

**Deployment mechanics, for next time:** both services were created via CLI (`vercel link` + `vercel --prod`, `render services create`). Render's CLI has no direct "edit an existing service's env vars" command in this version — the service had to be deleted and recreated once to fix `CLIENT_ORIGIN` (free plan, no cost either way, but worth knowing for future changes: check `render services update --help` again in case a newer CLI version adds this, before defaulting to delete-and-recreate). Render auto-deploys on push to `main` for changes under `server/` (its configured root directory) — a client-only commit correctly did not trigger a redundant server rebuild. Vercel does **not** auto-deploy on git push here (linked via CLI, not the GitHub App integration) — future client changes need `vercel --prod` run manually from `client/`, or the GitHub integration set up via `vercel git connect` if push-to-deploy is wanted later.
...
| 28 — Notification Engine | ✅ Done | Two sides, both real: `notifyThresholdCrossed()` (Stage 20, the acting user's personal confirmation) and `notifyStaffAndAdminOfAlert()` (new — every active Staff/Admin gets their own personal `Notification`, distinct from the shared `Alert` record in the Alert Center). Frontend: `NotificationBell` in `DashboardLayout`'s top bar (both mobile and desktop variants), live via SSE (`notification.created`, staff-scoped). `NotificationList` refactored to a controlled component (props instead of self-fetching) so both the public dashboard's inline list and the bell's dropdown share the same `useLiveNotifications()` hook. |
...
| 42 — Reports | ✅ Done | `/admin/reports` — generates a real "Collections Summary" report (total collections, estimated liters collected per bin, derived from actual `CollectionRecord` data, never fabricated). `Report` docs snapshot the computed data so history stays stable even if underlying records change later. |
| 43 — System Activity | ✅ Done | `/admin/activity` — audit trail. `logActivity()` wired into bin create/update/deactivate, user create/update/deactivate, collection recording, and alert acknowledge/resolve. Fire-and-forget (a logging failure never breaks the primary action). |
| 44 — Settings | ✅ Done | `/admin/settings` — the simulated waste-add percentage range (min/max) is now admin-configurable and **actually drives `addSimulatedWaste()`** (proven deterministically in tests: pinning min=max=X% produces exactly +X% on the next add). Does not touch the fixed 80/90/100 thresholds (Section 7). |
...
### Reports, System Activity, Settings (Stages 42-44) — Phases 1-5 now complete

Skipped **Phase 6** (Stages 45-49, the optional simulated-transaction/payment demo) — `PROJECT_MEMORY.md` Section itself marks these as build-only-if-the-final-academic-scope-requires-them and warns against letting them distract from the core waste-management objective. Not built unless explicitly requested.

**Bug pattern found again during this work** (third and fourth occurrence of the same class): `reportRepository.create()` and `settingsRepository.update()`/`getOrCreate()` both returned unpopulated documents, so "Generated by [[raw ObjectId]]" and a missing "by [name]" showed in the UI immediately after the action, before a refetch would have shown it correctly. Fixed the same way as the Stage 27 and Stage 35 instances — populate after create/update — and this time also **audited every remaining repository systematically** (`grep` for `.populate(` vs `.create(` across all repositories) rather than waiting to find each one by accident. Two were found NOT to need fixing (`alertRepository.create`, `issueReportRepository.create`) because their results are never rendered directly to the client. All four real instances now have regression tests asserting the *immediate* response is populated, not just a subsequent list fetch.
...
**Resolved 2026-08-15:** the real `MONGODB_URI` blocker is gone — see Section 36.1's update. Local dev now runs against the project owner's actual MongoDB Atlas cluster, not a throwaway in-memory instance. Data genuinely persists across server restarts (verified). The automated test suite deliberately still uses `mongodb-memory-server`, not Atlas — that's correct, not a gap.

**Resolved 2026-08-15:** deployed and live — see Section 36.9 for the full record (URLs, the code changes a real split deployment required, and what was verified against the live site rather than just a successful build). Client on Vercel, server on Render, both free tier, database is the same Atlas cluster local dev uses.

**Nothing currently outstanding from the project owner.** Everything previously listed here (Atlas credentials, deployment) has been provided and completed.
